...
if __name__ == "__main__":
    ...
    dir_yaml_interpret_low = os.path.join(os.environ.get("YAML_PATH_ANALYSIS"), "robustness_rotation_speed_low_interpret_vb06_resnet_crop.yaml")
    dir_yaml_interpret_high = os.path.join(os.environ.get("YAML_PATH_ANALYSIS"), "robustness_rotation_speed_high_interpret_vb06_resnet_crop.yaml")
    dir_yaml_interpret_mid = os.path.join(os.environ.get("YAML_PATH_ANALYSIS"), "robustness_rotation_speed_mid_interpret_vb06_resnet_crop.yaml")

    dir_yaml_interpret_low_pretrained = os.path.join(os.environ.get("YAML_PATH_ANALYSIS"), "robustness_rotation_speed_low_interpret_vb06_resnet_pretrained_crop.yaml")
    dir_yaml_interpret_high_pretrained = os.path.join(os.environ.get("YAML_PATH_ANALYSIS"), "robustness_rotation_speed_high_interpret_vb06_resnet_pretrained_crop.yaml")
    dir_yaml_interpret_mid_pretrained = os.path.join(os.environ.get("YAML_PATH_ANALYSIS"), "robustness_rotation_speed_mid_interpret_vb06_resnet_pretrained_crop.yaml")
    ...
    partition_loaders_low = prepare_dataloaders(dir_yaml_interpret_low, EXPERIMENT_ID, EXPERIMENT_NAME, None, None, transform=transform_cropping)
    ...
    transform_cropping = A.Compose([A.Crop(y_min=150, x_min=0, y_max=600, x_max=250), A.RandomCrop(height=224, width=224)])
    partition_loaders_high = prepare_dataloaders(dir_yaml_interpret_high, EXPERIMENT_ID, EXPERIMENT_NAME, None, None, transform=transform_cropping)
    # run_id and run_name are ignored because this preparation of data is not logged in mlflow

    # Run 3: initialize vb06 DataLoaders (higher end of parameter distribution)
    transform_cropping = A.Compose([A.Crop(y_min=150, x_min=0, y_max=600, x_max=250), A.RandomCrop(height=224, width=224)])
    partition_loaders_mid = prepare_dataloaders(dir_yaml_interpret_mid, EXPERIMENT_ID, EXPERIMENT_NAME, None, None, transform=transform_cropping)
    # run_id and run_name are ignored because this preparation of data is not logged in mlflow
